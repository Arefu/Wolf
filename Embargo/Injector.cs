...
namespace Embargo
{
    ...
    public static class Injector
    {
        ...
        private static bool InjectNow(int ProcToBeInjected, string Dll)
        {
            ...
            var LoadAddress = NaitiveMethods.GetProcAddress(NaitiveMethods.GetModuleHandle("kernel32.dll"), "LoadLibraryA");
            if (LoadAddress == StaticZero)
                return false;

            var Address = NaitiveMethods.VirtualAllocEx(Window, (IntPtr)null, (IntPtr)Dll.Length, (0x1000 | 0x2000), 0X40);
            ...
            if (NaitiveMethods.CreateRemoteThread(Window, (IntPtr)null, StaticZero, LoadAddress, Address, 0, (IntPtr)null) == StaticZero)
                return false;
